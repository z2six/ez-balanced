...
public class EzBalanceRarityEditScreen extends AbstractEzBalanceScreen {
    ...
    private String currentRarityId;
    ...
    public EzBalanceRarityEditScreen(Screen parent, EzBalanceConfig config, String currentRarityId) {
        super(Component.literal("Edit Rarity"));
        this.parent = parent;
        this.config = config;
        this.currentRarityId = currentRarityId;
    ...
    protected void init() {
        ...
        this.idBox = new EditBox(this.font, 24, 56, 180, 20, Component.literal("Rarity id"));
        this.nameBox = new EditBox(this.font, 214, 56, 220, 20, Component.literal("Rarity name"));
        this.colorBox = new EditBox(this.font, 444, 56, 110, 20, Component.literal("Hex color"));
        this.colorBox.setMaxLength(16);
    ...
    private void loadCurrentRarity() {
        EzBalanceRarityDefinition rarity = this.config.rarities.get(this.currentRarityId);
        if (rarity == null) {
            this.idBox.setValue("");
            this.nameBox.setValue("");
            this.colorBox.setValue("fc0553");
            return;
        }
    ...
    private void saveRarity() {
        String id = sanitizeId(this.idBox.getValue());
        if (id.isBlank()) {
            return;
        }

        String previousId = this.currentRarityId;
        EzBalanceRarityDefinition rarity = new EzBalanceRarityDefinition();
        rarity.id = id;
        rarity.name = this.nameBox.getValue().isBlank() ? id : this.nameBox.getValue();
        ...
        if (!previousId.isBlank() && !previousId.equals(id)) {
            this.config.rarities.remove(previousId);
            for (EzBalanceItemRule rule : this.config.items.values()) {
                if (previousId.equals(rule.rarityId)) {
                    rule.rarityId = id;
                }
            }
        }
    ...
    private void deleteRarity() {
        if (!this.currentRarityId.isBlank()) {
            this.config.rarities.remove(this.currentRarityId);
            for (EzBalanceItemRule rule : this.config.items.values()) {
                if (this.currentRarityId.equals(rule.rarityId)) {
                    rule.rarityId = "";
                }
            }
        }
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        drawLabel(graphics, "Rarity: " + (this.currentRarityId.isBlank() ? "<new>" : this.currentRarityId), 24, 28, true);
    ...
    private void syncSuggestionTarget() {
        ...
        if (focusedRow != this.suggestionTargetRow) {
    ...
    private void renderTextboxTooltips(GuiGraphics graphics, int mouseX, int mouseY) {
        ...
        if (renderTooltipForBox(
                graphics,
                this.idBox,
                mouseX,
                mouseY,
                "Internal rarity id",
                "Use a stable lowercase id like legendary."
        )) {
            return;
        }
        if (renderTooltipForBox(
                ...
                mouseX,
                mouseY,
                "Displayed rarity name",
                "This is what players see in the table and tooltip."
        )) {
            return;
        }
        if (renderTooltipForBox(
                ...
                mouseY,
                "Hex color",
                "Use a hex color like fc0553 or #fc0553."
        )) {
            return;
        }
        ...
        for (int index = 0; index < this.attributeBoxes.size(); index++) {
            if (renderTooltipForBox(
                    graphics,
                    this.attributeBoxes.get(index),
                    mouseX,
                    mouseY,
                    "Attribute id",
                    "Type an attribute id. Matching ids appear below and can be scrolled."
            )) {
                return;
            }
            if (renderTooltipForBox(
    ...
    private boolean renderTooltipForBox(GuiGraphics graphics, EditBox box, int mouseX, int mouseY, String title, String body) {
        ...
        graphics.renderTooltip(
                this.font,
                List.of(Component.literal(title), Component.literal(body)),
