...
public class EzBalanceItemGroupEnchantmentScreen extends AbstractEzBalanceScreen {
    ...
    public EzBalanceItemGroupEnchantmentScreen(Screen parent, EzBalanceConfig config, String itemGroupId) {
        super(Component.literal("Item Group Enchant Rules"));
        this.parent = parent;
        this.config = config;
    ...
    protected void init() {
        this.clearWidgets();
        this.searchBox = new EditBox(this.font, 24, 54, 300, 20, Component.literal("Search enchantments"));
        this.searchBox.setResponder(value -> refreshEnchantments());
        this.addRenderableWidget(this.searchBox);
        this.addRenderableWidget(customButton("Clear Rules", 336, 54, 118, 20, button -> clearRules()));
        this.addRenderableWidget(customButton("Back", this.width - 90, this.height - 28, 70, 20, button -> this.onClose()));
        refreshEnchantments();
    }

    private EzBalanceItemGroupDefinition getItemGroup() {
        return this.config.itemGroups.computeIfAbsent(this.itemGroupId, key -> new EzBalanceItemGroupDefinition(key, key));
    }
    ...
    private void toggleEnchantment(String enchantmentId) {
        ...
        if (itemGroup.allowedEnchantments.contains(enchantmentId)) {
            itemGroup.allowedEnchantments.remove(enchantmentId);
        } else {
    ...
    private boolean isInsideForceDisabledToggle(double mouseX, double mouseY) {
        return mouseX >= 24 && mouseX <= 240 && mouseY >= 80 && mouseY <= 94;
    }
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        drawLabel(graphics, "Item Group Enchant Rules", 24, 24, true);
        drawLabel(graphics, "Item group: " + this.itemGroupId, 24, 36, false);
        renderCheckbox(graphics, 24, 82, getItemGroup().forceDisabledEnchants);
        drawLabel(graphics, "Force disabled enchants", 40, 82, false);
    ...
    private void renderRows(GuiGraphics graphics, int mouseX, int mouseY) {
        ...
        for (int row = 0; row < visibleRows; row++) {
            ...
            boolean checked = itemGroup.allowedEnchantments.contains(enchantmentId);
            ...
            renderCheckbox(graphics, 32, y + 6, checked);
    ...
    private void renderCheckbox(GuiGraphics graphics, int x, int y, boolean checked) {
        ...
        graphics.fill(x, y + 9, x + 10, y + 10, checked ? COLOR_ACCENT : COLOR_BORDER);
        graphics.fill(x, y, x + 1, y + 10, checked ? COLOR_ACCENT : COLOR_BORDER);
        graphics.fill(x + 9, y, x + 10, y + 10, checked ? COLOR_ACCENT : COLOR_BORDER);
        if (checked) {
